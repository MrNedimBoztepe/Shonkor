...
* **Ingested repository content is untrusted regardless of who triggered the scan.** A locally cloned third-party repo is no more trustworthy than one pushed via webhook, so there is no "operator-driven means trusted" boundary. The first-party security post-processors (`FirstPartyPostProcessors`, notably `SuspiciousContentPostProcessor`, which flags prompt-injection-style text later handed verbatim to an LLM) are therefore appended by `GraphIndexScanner`'s **constructor**, not by each call site — they run on **every** full scan by construction (today: CLI index, index endpoint, and a webhook push without a file list), and there is deliberately **no opt-out flag**. As a whole-graph phase they run on `ScanDirectoryAsync` only — never on a single-file reindex, which is why drift reconcile and the MCP edit-loop paths do *not* produce these diagnostics. A plugin **cannot displace them through the post-processor pipeline**: diagnostics are keyed by post-processor `Name` and a scan *replaces* the set for a name, so caller-supplied post-processors whose name collides with a first-party one are dropped. This is a pipeline guarantee, not a sandbox — an activated plugin runs in-process and the activation itself is the trust gate (see the plugin bullet above). **Execution is not the same as a result, so the gap between them is reported (#353):** a failure while scanning one node no longer discards the rest of the run, and a pass that does not complete leaves a machine-readable `postprocessor.incomplete` diagnostic (severity `Error`, written under the processor's own diagnostic source so the next successful scan clears it) instead of only a log line the web UI never shows. That is what makes the promise falsifiable: "no `security.*` diagnostics" now means *checked and clean* rather than *possibly never ran* — **unless a `postprocessor.incomplete` diagnostic is present**. The condition matters for filtered readers: `GetDiagnosticsAsync` matches the code exactly, so a consumer querying only `security.suspicious-instruction-in-content` (e.g. `GroundingPrep`) never sees the marker and must check for it separately. The unfiltered views (MCP `get_diagnostics`, the dashboard) show it alongside everything else.
