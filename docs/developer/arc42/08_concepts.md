...
* **Embedding source**: the vector is computed from a structured **code document** (`type + name + signature + summary + bounded body`) by default, not just the AI summary — configurable via `Embedding:Source` (`code` | `summary`). This measurably improves natural-language ("intent") retrieval over embedding the summary alone. Each vector's dimension is stored, so a model change flags stale vectors for re-embedding (`MarkStaleEmbeddingsForReembedAsync`, once per process) rather than silently dropping them from vector search.
