...
* **Ingested repository content is untrusted regardless of who triggered the scan.** A locally cloned third-party repo is no more trustworthy than one pushed via webhook, so there is no "operator-driven means trusted" boundary. The first-party security post-processors (`FirstPartyPostProcessors`, notably `SuspiciousContentPostProcessor`, which flags prompt-injection-style text later handed verbatim to an LLM) are therefore appended by `GraphIndexScanner`'s **constructor**, not by each call site — they run on **every** full scan (CLI, index endpoint, webhook, drift, MCP) by construction, and there is deliberately **no opt-out flag**. As a whole-graph phase they run on `ScanDirectoryAsync` only, never on a single-file reindex. A plugin **cannot replace or overwrite them**: diagnostics are keyed by post-processor `Name` and a scan *replaces* the set for a name, so caller-supplied post-processors whose name collides with a first-party one are dropped.
