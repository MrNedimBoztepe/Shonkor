...
* **JavaScript/TypeScript Parser (plugin)**: Not part of the host — the installable `shonkor-typescript` plugin supplies it. A thin `IFileParser` adapter drives a **Node sidecar running the real TypeScript Compiler API**, so `.ts/.tsx/.js/.jsx` are analyzed with genuine TS semantics (symbols, resolved relative imports) — which makes **Node ≥ v18 a prerequisite** for that path (see [chapter 2](02_architecture_constraints.md) and the [Setup Guide](../../user/setup_guide.md#step-3-node-runtime-for-jsts-analysis)). When Node is unavailable or a parse times out, the plugin degrades to its own **private, in-plugin Esprima** fallback (`EsprimaFallbackParser`) instead of stopping — reported per file as a *log* warning (an `IFileParser` has no diagnostics channel), while the queryable `Info` diagnostic about the skipped cross-file semantic pass comes from `TypeScriptSemanticLinker` — the host itself takes no dependency on Esprima: there is no loose `Esprima.dll` next to the host binaries and no entry in the host's dependency manifest, so no host load path exists. The fallback's copy travels inside the plugin package and is loaded exclusively in the plugin's own `AssemblyLoadContext`.
