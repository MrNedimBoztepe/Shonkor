...
/// <summary>
/// How the MCP relay loads the active plugins for a single request. Defaults to
/// <see cref="AssemblyPluginLoader.LoadActive(string, IPluginHost?)"/>; this seam exists so a test can
/// substitute a known plugin parser without a plugin on disk or a live sidecar — the non-tenant-locked
/// relay branch is otherwise unreachable over the HTTP pipeline (<see cref="Middleware.ApiKeyMiddleware"/>
/// always tenant-locks <c>/api/mcp</c>), so the endpoint's plugin-parser merge could only be exercised by
/// re-implementing it in a test. Overriding this in DI lets a test drive the real endpoint handler instead.
/// </summary>
public delegate AssemblyPluginLoadResult RelayPluginLoader(string workspacePath, IPluginHost? host);
...
public static class McpEndpoints
{
    public static void MapMcpEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        // Registered as a named method (not an inline lambda) so a test can drive the exact handler the
        // endpoint runs — that is what makes the plugin-parser merge wiring (#315) verifiable: a regression
        // that drops the merge here turns the endpoint test red, not just the isolated helper test.
        group.MapPost("/relay", RelayAsync)
            .WithName("RelayMcpMessage")
            .WithSummary("Relays an MCP JSON-RPC message to the backend GraphRAG server.");
    }

    internal static async Task<IResult> RelayAsync(
        HttpContext context,
        ...
        [FromServices] ContextCapsuleSynthesizer synthesizer)
    {
        ...
        // The plugin loader is a DI-overridable seam (default: the real assembly loader). Only a test
        // substitutes it, to supply a known plugin parser and prove the merge below actually runs.
        var loadActivePlugins = context.RequestServices.GetService<RelayPluginLoader>()
            ?? new RelayPluginLoader(AssemblyPluginLoader.LoadActive);
        ...
        if (!isTenantLocked && config != null && PluginsEnabled(config))
        {
            ...
            pluginLoad = loadActivePlugins(projectManager.WorkspacePath, pluginHost);
        }
