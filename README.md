...
>
> And we tested whether a chunk pipeline could simply *copy* that advantage: `--compare-rag --enrich-baseline` hands the baseline's chunks the very same node summaries and scores it **paired, in one process, against the same cases and token budget** as the plain baseline (#189/#284). It does **not** close the gap — the two arms differ by about **three cases of 33, inside the noise band**. Pasting the summary text into flat chunks doesn't make them win, because a chunk's vector is diluted by 40 lines of code and FTS5's implicit-AND still can't match a full-sentence query against a summary that lacks its function words. So the summary is not a transferable driver a chunk retriever could bolt on: the win stays with the **node as the indexed unit and the edges around it**, not with a string. (Method and the confidence-interval reasoning: [arc42 §5](docs/developer/arc42/05_building_block_view.md).)
