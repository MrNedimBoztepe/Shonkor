...
# constants: template id (lower) → "<X>Constants" class names; models: constants class → model classes
declare -A CONST_OF_ID M_CLASSES_OF_CONST M_PATH M_TOKEN M_FULL
...
for rec in "${M_ORDER[@]}"; do
  ...
  i=$((i+1)); M_TOKEN[$m]=$(printf 'Model-%0*d' "$MW" "$i"); M_PATH[$m]=$f; M_FULL[$m]="$ns.$m"
  M_CLASSES_OF_CONST[$c]="${M_CLASSES_OF_CONST[$c]:-} $m"
done
...
for p in "${R_PATHS[@]}"; do
  ...
  read -r -a models <<< "${M_CLASSES_OF_CONST[${consts[0]}]:-}"
  ...
  USED_MODEL[$m]=1
  same="no"; [ "$(lower "${T_NAME[$tp]// /}")" = "$(lower "$m")" ] && same="yes"
  n=$((n+1))
...
grep -qxF 'bench/' "$ROOT/.git/info/exclude" 2>/dev/null || printf 'bench/\n' >> "$ROOT/.git/info/exclude"
{
  ...
  for rec in "${M_ORDER[@]}"; do IFS=$'\t' read -r f ln m c ns <<< "$rec"; entry "${M_TOKEN[$m]}" model path "$f" name "$m" fullName "${M_FULL[$m]}"; done
  printf '\n  }\n}\n'
